---
title: "背景模糊" 
description: "將背景模糊效果套用在元素"
featureVersion: 'v2.1+'
---

import { Variants } from '@/components/Variants'
import { Disabling } from '@/components/Disabling'

export const classes = {
  plugin: function () {
    return function ({ addUtilities }) {
      addUtilities({
        '.backdrop-blur-0': { '--tw-backdrop-blur': 'blur(0)' },
        '.backdrop-blur-sm': { '--tw-backdrop-blur': 'blur(4px)' },
        '.backdrop-blur': { '--tw-backdrop-blur': 'blur(8px)' },
        '.backdrop-blur-md': { '--tw-backdrop-blur': 'blur(12px)' },
        '.backdrop-blur-lg': { '--tw-backdrop-blur': 'blur(16px)' },
        '.backdrop-blur-xl': { '--tw-backdrop-blur': 'blur(24px)' },
        '.backdrop-blur-2xl': { '--tw-backdrop-blur': 'blur(40px)' },
        '.backdrop-blur-3xl': { '--tw-backdrop-blur': 'blur(64px)' },
      })
    }
  }
}

## 用法


將 `backdrop-blur-{amount?}` 功能跟 `backdrop-filter` 功能一起使用即可將一個元素的背景變模糊

```html rose
<template preview class="p-4">
  <div class="flex justify-center content-center flex-wrap h-24 bg-stripes2 bg-stripes-rose-500 rounded-lg">
    <div class="backdrop-filter backdrop-blur-sm w-1/4 h-20 text-white text-2xl font-extrabold rounded-md border-rose-300 border-2 flex items-center justify-center bg-rose-500 bg-opacity-0">
      DEMO
    </div>
  </div>
</template>

<div class="backdrop-filter **backdrop-blur-sm** ...">
  <!-- ... -->
</div>
```

## 響應式

如要將一個元素的背景模糊控制在特定斷點，在任何一個既有背景模糊璃功能的前綴加上 `{screen}:` 就好。例如，在中型或以上的螢幕尺寸，將 `md:backdrop-blur-lg` 加入 `backdrop-blur-lg` 功能。

```html
<div class="backdrop-filter backdrop-blur-sm **md:backdrop-blur-lg** ...">
  <!-- ... -->
</div>
```

想知道更多有關 Tailwind 響應式設計功能的訊息，請參考 [響應式設計] (/docs/responsive-design) 文件

## 客製化

### 應用在變化模式 Variants

<Variants plugin="backdropBlur" name="backdrop blur" />

### 停用

<Disabling plugin="backdropBlur" name="backdrop blur" />
