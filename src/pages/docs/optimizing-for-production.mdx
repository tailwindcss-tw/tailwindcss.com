...
title: Optimizing for Production (優化生產模式)
...
<p>在預設配置下的開發者環境架構中， Tailwind CSS 未壓縮的檔案大小為 {stats.default.original}，使用 <a href="https://www.gnu.org/software/gzip/">Gzip</a> 來壓縮加上 minified 後得到 {stats.default.gzipped}，使用 <a href="https://github.com/google/brotli">Brotli</a> 壓縮更能達到 {stats.default.brotlified}</p>
...
為了提升更有效率的開發體驗，Tailwind 產出了上千個 utility class，其中可能有一大部分你根本不會使用到。
...
你可以把 Tailwind 想像成一大箱樂高，將其傾倒至地上後取出需要的部分來組合，直到完成後，將全部沒有用到的積木通通放回去箱子。
...
**當部屬生產模式時，你應該永遠使用 Tailwind 的 `purge` 選項，將未使用的樣式 tree-shake 並最小化至最終的編譯大小。** 當 Tailwind 移除未使用的樣式後，最終壓縮後的 CSS 檔案幾乎很難超過 10kb。
...
使用 `purge` 功能前，必須要優先了解它的運作方式以及建構正確的 mental model ，確保你永遠不會在部屬生產模式時意外地移除重要的樣式。
...
現在只要在 `NODE_ENV` 設定為 `production` 的情況下編譯 CSS，Tailwind 將會自動從你的 CSS 中 purge 未使用的樣式。
...
預設情況下，Tailwind 會保留所有用於 HTML 基本元素上的 CSS 樣式，例如 `html`、`body`、`p`、`h1` ...等等標籤上的樣式。這是為了將過於 purge 的情形最小化的作法，比如說，使用 markdown 檔案 (此種檔案並不實際存在 `h1` 標籤)，或是使用框架時可能會將 document shell (包含 `html` 和 `body` 標籤) 隱藏在某處的 vendor 目錄 (例如 Next.js)。
...
一般來說，我們認為弊大於利並不建議這樣設定，但我們不是警察，你想怎樣就怎樣。
...
預設情況下，Tailwind 會 purge `base`、`components` 和 `utlities` 這些 layers 裡面全部的樣式。如果你想要改動此設定，可以指定 `layers` 的值來手動指定你想要移除的 Layers。
...
預設情形下，Tailwind 僅會移除其自行生成樣式中未使用的部分，或是已經被明確包在 `@layer` 的 directive 裡面。並 _不會_ 移除你自行額外加入專案裡的第三方插件中未使用的 CSS 樣式，例如 datepicker。
...
這是為了避免意外地移除你可能需要但沒有直接出現在樣板中的樣式，例如你從 `node_modules` 資料夾深處之中引用一部分 dependency 的 class。
...
如果你因為某種原因無法使用 PurgeCSS，你也可以從 [你的 configuration 檔案](/docs/configuration) 裡的刪除未使用的值來減少 Tailwind 的足跡。
...
我們不希望你撰寫新的 CSS，只因為我們沒有提供足夠的 padding helpers，或是你想要一個橘色系的色彩計畫但我們卻只提供藍色給你。
...
不過你可能需要權衡一下：預設架構下的檔案會比有客製化 configuration 檔案的架構來的肥大許多。
...
預設 theme 提供高達 [84 色](/docs/customizing-colors) 的 backgrounds、borders、text 和 placeholders，而這些各自都包含了 `hover:` 和 `focus:` 的 variants，甚至還有預設的六個螢幕尺寸 responsive variants。
...
預設情況下將會產生 _上千個_ 與這些顏色有關的 classes，它們甚至占了最終檔案大約一半的大小。
...
下面列舉了使用較少 color palette 的最終檔案大小結果：
...
由於幾乎每個 Tailwind utility 都是從每個螢幕尺寸複製而來的，使用更少的螢幕尺寸也可以為整體檔案大小帶來巨大的影響。
...
### 停用未使用的 core plugins 和 variants
...
如果預期在你的專案中完全不會使用到某些 utility plugin，你可以在設定檔中把 `corePlugins` 設為 `false` 來完全停用它們：
...
如果你只會用到少量的 utilities，可以在 `corePlugins` 設定欲保留的 utility plugins 陣列。
...
上面的範例將會停用除了 margin 和 padding 之外的 utilities。
...
如果你只需要不包含響應式的 utility，將其 variants 設為空陣列可以減少產生 83% 的 classes。
...
比起限制你的 color palette 或使用較少的斷點，這些只是小小的勝利，但堆疊起來的檔案大小仍然不可小覷。
