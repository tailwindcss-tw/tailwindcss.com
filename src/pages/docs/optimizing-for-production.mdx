...
這是為了避免意外地移除你可能需要但沒有直接出現在樣板中的樣式，例如你從 `node_modules` 資料夾深處之中引用一部分依賴檔 (dependencies) 的 class。
...
如果你預期在專案中不會使用到某些 utility plugin，可以在設定檔中把 `corePlugins` 設為 `false` 來完全停用它們：
...
如果你不需要響應式功能，將其 variants 設為空陣列可以減少產生 83% 的 classes。
