...
如要讓元素的背景負片有響應式效果，可在現有背景負片功能前加上前綴詞 `{screen}:` 。例如：中型或以上的螢幕尺寸使用 `md:backdrop-invert-0` 以套用 `backdrop-invert-0` 功能。
...
你可以在`tailwind.config.js`的`theme`區塊中，使用`backdropInvert`這個 key 來客製化你要的`backdrop-invert`效果。
