---
title: "Backdrop Invert (背景反轉)"
description: "將背景反轉濾鏡套用在元素"
featureVersion: 'v2.1+'
---
...
不論是否要將元素背景渲染成反轉，將 `backdrop-invert` 或 `backdrop-invert-0` 跟 `backdrop-filter` 功能搭配使用。
...
如要將元素的背景反轉控制在特定斷點，在任何一個既有背景反轉功能的前綴詞加上 `{screen}:` 。例如，使用 `md:backdrop-invert-0` 時，只有中型或以上的螢幕尺寸有 `backdrop-invert-0` 效果。
