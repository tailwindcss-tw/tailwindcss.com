...
description: "根據當前斷點固定元素(elemnet)的寬度的元件"
---
...
`container` class設定元素的`max-width`來匹配當前斷點的`min-width`。這對於想要依照固定螢幕尺寸而非動態

<!-- Note that unlike containers you might have used in other frameworks, **Tailwind's container does not center itself automatically and does not have any built-in horizontal padding.** -->
請注意這可能跟你以往所使用的框架的 container不一樣。 **Tailwind的 container 不會自動置中，也沒有內置的水瓶內間距。**
...
`container` class也包含響應式變化模式。像是預設的`md:container`能讓你的某些東西在某個斷點或者某些斷點表現得像一個 container。
...
預設居中 container。在你的config文件裡`theme.container`中設定`center`選項為`true`。
...
預設加上水平內間距，請在你的config文件裡`theme.container`中設定你所想要的數值在`padding`選項。
...
如果你沒計劃在你的專案裡使用`container` class，你可以在你的 config文件裡的 `corePlugins`物件內的 `container` 屬性設定為`false`來停用。
