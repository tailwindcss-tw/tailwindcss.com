...
description: "根據當前斷點固定元素 (element) 寬度的元件"
---
...
`container` class 設定元素的 `max-width` 來匹配當前斷點的 `min-width`，這對於想要依照固定螢幕尺寸而非動態調整的方式很有用。

<!-- Note that unlike containers you might have used in other frameworks, **Tailwind's container does not center itself automatically and does not have any built-in horizontal padding.** -->
請注意，這可能跟你以往所使用的框架的容器 (container) 不一樣。 **Tailwind 的容器 (container) 不會自動置中，也沒有內置的水平內間距。**

<!-- To center a container, use the `mx-auto` utility: -->
使用 `mx-auto` 功能來置中容器 (container)。
...
想要預設容器 (container)一開始就置中。在你的設定檔裡 `theme.container` 中設定 `center` 選項為 `true` 。
...
假如你想在不同的斷點上設定不同的內間距，可以使用一個物件 (object)來在不同的斷點覆蓋 `default` 的值。
...
倘若你想停用響應式變化模式，可以在你的 `tailwind.config.js` 文件裡的 `variants` 內將 `container` 設定為空陣列。
...
如果你不打算在你的專案裡使用 `container` class，你可以在你的設定檔裡的 `corePlugins` 物件內的 `container` 屬性設定為 `false` 來停用。
