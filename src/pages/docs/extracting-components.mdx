...
對於小型元件，像是按鈕、表單元件，跟建立 CSS class 相比，建立模板或是 JavaScript 元件可能會太過於沈重。
...
為了避免非預期的意外，我們建議你將自定義元件的樣式使用 `@layer components { ... }` 指令包住來告知 Tailwind 這些樣式屬於哪一個層級：
...
在 [元件插件](/docs/plugins#adding-components) 了解更多。
