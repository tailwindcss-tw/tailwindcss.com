...
使用 `@tailwind` 指令可以把 Tailwind 的 `base`、`components`、`utilities` 和 `screens` 樣式插入到你的 CSS 中。
...
將自定義的 CSS 使用 `@layer` 囊括住也可以告知 Tailwind 在清除不必要的 CSS 時會一併檢查這些樣式，詳細可以閱讀我們的 [生產環境優化](/docs/optimizing-for-production)。
...
由於 Tailwind 使用 [巢狀語法](/docs/colors#nested-object-syntax) 來定義預設的調色盤，請確保使用小數點來使用這些巢狀結構的顏色。  
