...
有了 Tailwind，你就可以用 Tailwind ==已經存在的、預先設計好的 class 名稱== 直接在 html 修改你的樣式。
...
- [最大寬度 (max-width)](/docs/max-width) and [margin](/docs/margin) utilities (`max-w-sm` and `mx-auto`) to constrain the card width and center it horizontally
