...
如果你想根據在 `tailwind.config.js` 檔案中定義的斷點生成自訂功能的[響應式 variants](/docs/responsive-design)，請將自訂功能包在 `@variants` 指令中，並且在 variants 清單中加入 `responsive`：
...
如果你想產生自訂功能 class 的 [深色模式 variants](/docs/dark-mode)，首先要確定 `tailwind.config.js` 檔案中 `darkMode` 需設置為 `media` 或 `class`：
...
如果你想為自訂功能建立[狀態 variants](/docs/hover-focus-and-other-states)，請將自訂程序包裝在 `@variants` 指令中，並列出要啟用的variants：
...
要了解更多資訊，請見[狀態 variants](/docs/hover-focus-and-other-states)。
