...
這個功能只是一個在「除了第一個之外的所有項目」群組內增加外距的捷徑，它並非是設計來處理包裝過的排版，像是網格 (grids)、「子元素以非 DOM 自然排列的自定義複雜順序呈現」等複雜案例。
...
如要讓子元素的間距有響應式效果，可在現有元素間距功能前加上前綴詞 `{screen}:`。例如：使用 `md:space-x-8` 可以在中型或以上的螢幕尺寸套用 `space-x-8` 的功能。
