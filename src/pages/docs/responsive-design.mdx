...
在 Tailwind 上的每一個功能 class 可以有條件地應用於不同的斷點，這讓您可以輕鬆地建立複雜的響應式介面而不需要離開 HTML。
...
欲增加一個功能但僅生效在某個特定斷點上，您只需要加入該斷點的前綴詞，並在後面加上 `:` 即可。
...
這適用於**框架中的任何一個功能 class**，代表您可以給定斷點來改變任何東西，甚至是字元間距或滑鼠游標的樣式。
<!-- This works for **every utility class in the framework**, which means you can change literally anything at a given breakpoint — even things like letter spacing or cursor styles. -->

接下來是一個簡單的行銷頁面元件的範例，該範例在小螢幕上使用堆疊的排版，大螢幕上使用並排的排版 *(調整您的瀏覽器大小來檢視它的行為)*
...
在這個範例裡面我們僅使用一個斷點，但是您也可以使用其他 `sm`、`lg` 或 `xl` 等響應式前綴來簡單的客製化這個元件。
...
在預設情況下，Tailwind 使用了行動裝置優先的斷點系統，如同其它您可能已經熟悉的框架，例如 Bootstrap。
...
這種方式最讓人驚豔的是針對手機的樣式設計，您需要使用沒有前綴詞的功能，而不是 `sm:` 的前綴詞。不要將 `sm:` 視為 「小螢幕上」，而應該將其視為「小螢幕*斷點*處」
<!-- Where this approach surprises people most often is that to style something for mobile, you need to use the unprefixed version of a utility, not the `sm:` prefixed version. Don't think of `sm:` as meaning "on small screens", think of it as "at the small *breakpoint*". -->

<TipBad>不要使用 <code className="text-sm font-bold text-gray-800">sm:</code> 來定位行動裝置</TipBad>
...
<TipGood>使用沒有前綴詞的功能來定位手機，並在較大的斷點覆蓋它們</TipGood>
...
Tailwind 的斷點僅包含 `min-width` 而沒有 `max-width`，這代表任何功能您新增在較小斷點上，將會同時生效在比其還大的所有斷點。
...
如果您只想在某個斷點上應用某項功能，只需要在更大的斷點上新增另一個的功能，藉此來*抵銷*前一項功能的效果。
...
請注意，**不必**在 `sm` 或 `xl` 的斷點上指定背景顏色，您只需要指定一個功能何時*開始*生效，而不是何時結束。
...
在您的 `tailwind.config.js` 檔案中，可以完全地客製化斷點。
