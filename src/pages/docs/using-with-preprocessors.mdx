...
description: Tailwind 跟一般 CSS 預處理器像是 Sass 、 Less 、 Stylus 搭配使用導覽
---
...
Tailwind是 PostCSS 插件，因此你可以任意跟其他預處理器像是 Sass 、 Less 、 Stylus 搭配使用。就像你可以跟像[Autoprefixer](https://github.com/postcss/autoprefixer)的 PostCSS 插件一起使用一樣。
...
要將 Tailwind 跟其他像 Sass、Less或 Stylus 之類的預處理器搭配使用， 你將需要增加一個建置的步驟，才能透過 PostCSS 執行你預處理的 CSS。
...
要了解如何使用 Tailwind 搭配預處理器最重要的就是要知道**Sass、Less或 Stylus 之類的預處理器是在 Tailwind 執行前分別執行**。舉例來說，你不能將 Tailwind 的 `theme()` 函式結果輸出到 Sass 顏色函式，因為要在 Sass 完成編譯並輸出給 PostCSS 後，`theme()` 函式才會被執行。

{/* <TipBad>Won't work, Sass is processed first</TipBad> */}
<TipBad>無作用， Sass先被處理</TipBad>
...
反而，將一般的 media query 跟 `theme()` 搭配使用來標示螢幕尺寸，或是不要將 `@screen` 放在巢狀裡。
...
儘管如此，這樣會造成巨大的損失，因為**你在 `@css` 區塊中，無法使用任合 Stylus 功能**。
