...
將 `backdrop-hue-rotate-{amount}` 功能跟 `backdrop-filter` 搭配使用，即可模糊元素。
...
如要將元素的背景色相控制在特定斷點，在任何一個既有背景色相功能的前綴詞加上 `{screen}:` 。例如，使用 `md:backdrop-hue-rotate-60` 時，只有中型或以上的螢幕尺寸有 `backdrop-hue-rotate-60` 效果。
