...
我們沒有草率的做出相容性的變更，並且我們努力地確保升級的路徑盡可能的相對簡單。對於大部分的專案，升級將會在 30 分鐘內完成。
...
在 Tailwind CSS v2.0 新加入了 `2xl` 頁面斷點，這將會影響所有你已經使用的 `container` 功能。請注意！如果這功能影響了你，請移除 `2xl` 這個頁面斷點，並將下列的 `screens` 覆寫到你已經擁有的螢幕斷點：
...
在 v1.0 版本裡，Tailwind 會忽略 `preserveHtmlElements` 在你自訂 PurgeCSS 萃取器裡。現在這個選項在 v2.0 版本裡，不會再被忽略，只是你如果不需要，記得在選項裡取消。
