...
Tailwind CSS v2.0 是自從 2019 年 5 月發表 v1.0 以來，第一個新的主要版本，因此它會帶來一些很小的與舊版本上相容性的變更。
...
我們沒有草率的做出相容性的變更，並且我們努力地確保升級的路徑盡可能的相對簡單。
對於大部分的專案，升級將會在 30 分鐘內完成。
...
使用更新 Tailwind 以及安裝 PostCSS 和 autoprefixer :
...
在 2.0 版本之前，我們努力的讓所有在 Tailwind 的功能在 IE 11 上都能正常的使用。但這對我們來說在開發及維護上造成相當大的負擔，以及增加了在打包之後的總體大小(即使清理了所有未使用的樣式)，所以我們決定在 2.0 版本不再支援 IE 11。 
...
在 Tailwind CSS v2.0 新加入了 `2xl` 頁面斷點，這將會影響所有你已經使用的 `container` 功能。 

請注意！如果這功能影響了你，請移除 `2xl` 這個頁面斷點，並將下列的 `screens` 覆寫到你已經擁有的螢幕斷點：
...
**請注意！ 如果你已經有自己的調色盤，可以放心地跳過這個步驟，不用做任何修改**
...
如果你正在使用 v1 版本中的預設調色盤，那你應該將以下的預設顏色，覆蓋到你的現在用的設定文件。
...
**請注意！ 如果你已經有自己的字體大小，可以放心地跳過這個步驟，不用做任何修改**
...
另外，你也可以在每個你有使用到行高( lineHeight )的部分，新增 `leading` 這項功能。
...
在 Tailwind CSS v1.0 中， `theme` 的 `extend` 結構是會向上合併的，因此這樣，以下這個設定可能會覆蓋掉所有的紫色。
...
在 Tailwind CSS v2.0 中，結構是會向下合併的，所以根據上面這個設定產生出預設的 `purple-100` 到 `purple-900` 以及你新增的 `purple-light` 、 `purple-medium` 以及 `purple-dark` 色調。
...
這在大部分的狀況是有用的， 但如果你的顏色的結構是需要向上合併的話，還是會有問題, 所以你可能要自己手動修改你所新增的，將它往上跟預設的顏色合併在一起。
...
現在, class 會照他在原生 CSS 中所出現的順序所出現。
...
在 Tailwind CSS v1.0 版本中， 你可以 `@apply` 不帶有你自訂前綴詞的功能 class 。
...
在 v2.0 版本中不再支援這個方式, 所以如果你有自訂前綴詞的話 (像是 `tw-`) ，你需要確保在你有使用到 `@apply` 前綴詞的功能 class 的地方，加入你的自訂前綴詞。
...
你可以利用這個正則表達式來快速找到所有 `@apply` 中所出現的前導 `.` 並且刪除他。
...
<!-- truncate` utility is now part of the `textOverflow` core plugin, so if you had enabled any extra variants (like `group-hover`) for the `wordBreak` plugin in order to use them with the `truncate` class, you'll want to enable them for `textOverflow` now as well or instead: -->
 `truncate` 現在已經是 `textOverflow` 的一部分核心，
 
 所以你可能為了使用 `wordBreak` 變化模式而打開的 (像是 `group-hover`) 因為他們裡面包含 `truncate` 功能性 class。
 
 現在你也需要為 `textOverflow` 而打開：
...
如果你還是需要他們在，就是那些會需要在舊版本的 iOS 上執行的專案，你可以在自訂的 `utilities` 上增加。
...
## 到任何一個有使用到 space 或 divide 元素 template 標籤新增 hidden 
...
如果你在 options 有用到設定 PurgeCSS 的部分，你會需要更新 `options` 裡的設定。
<!-- For example, if you were using whitelist, you'll want to update this to safelist: -->
...
## 如果你有自訂 PurgeCSS 萃取器，請取消 preserveHtmlElements 選項
...
在 v1.0 版本裡，Tailwind 會忽略 `preserveHtmlElements` 在你自訂 PurgeCSS 萃取器裡。

現在這個選項在 v2.0 版本裡，不會再被忽略，只是你如果不需要，記得在選項裡取消。
...
如果你在 `tailwind.config.js` 裡有自訂前綴, Tailwind v2.0 會幫你自動加入前綴到 keyframes 裡的 `theme`。
<!-- If you are referencing any configured keyframes in custom CSS, you'll want to make sure you add your prefix:  -->
但是如果你有任何自訂的 CSS 裡用到 keyframes， 請記得加入你的自訂前綴
...
這只會發生在你使用自訂前綴且在你的自訂 CSS 裡才會受影響，如果這會影響到這個星球上兩個人以上，我絕對會嚇死。
