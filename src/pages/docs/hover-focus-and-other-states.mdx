...
如果你需要使用 Tailwind 所不支援的狀態，可以參閱 [建立自定義變化模式插件](#-3) 來擴充所需要的變化模式。
...
新增 `active:` 前綴詞來啟用當一個元素的狀態是 active 時的 active 功能。
...
新增 `hover:` 前綴詞讓 `prefers-reduced-motion` 符合 `no-preference` 時才啟用該功能。

<!-- For example, this button will only animate on hover if the user hasn't enabled "Reduce motion" on their system. -->
舉例來說，這個按鈕只有在使用者沒有啟用 "Redice motion" 時並且 hover 時才會有動畫效果。
...
新增 `checked:` 前綴詞讓 radio 或 checkbox 為 checked 時才啟用該功能。
...
新增 `first:` 前綴詞讓元素為其父元素的 first-child 時才啟用該功能。這在元素透過某些迴圈方式產生時特別有用。
...
新增 `last:` 前綴詞讓元素為其父元素的 last-child 時才啟用該功能。這在元素透過某些迴圈方式產生時特別有用。
...
新增 `odd:` 前綴詞讓元素為其父元素的 odd-child 時才啟用該功能。這在元素透過某些迴圈方式產生時特別有用。
...
新增 `even:` 前綴詞讓元素為其父元素的 even-child 時才啟用該功能。這在元素透過某些迴圈方式產生時特別有用。
