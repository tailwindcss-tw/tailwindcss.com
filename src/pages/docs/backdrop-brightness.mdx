...
將 `backdrop-brightness-{amount?}` 功能跟 `backdrop-filter` 功能一起使用即可控制元素的背景亮度。
...
如要將一個元素的背景亮度控制在特定斷點，在任何一個既有背景亮度功能的前綴加上 `{screen}:` 就好。例如，在中型或以上的螢幕尺寸，將 `md:backdrop-brightness-150` 加入 `backdrop-brightness-150` 功能。
