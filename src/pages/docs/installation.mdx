...
如果你要用 CDN 方式引入 Tailwind，[點我快速帶你到那邊去][CDN]！
...
這時候會產生一個預設值的 tailwind配置檔 `tailwind.config.js` 在你的專案根目錄：

```js
// tailwind.config.js 設定檔
module.exports = {
  purge: [],
  darkMode: false, // 也可能是 'media' 或 'class'
  theme: {
    extend: {},
  },
  variants: {},
...
在用 CDN 引入之前請注意，因為 Tailwind 有很多特色功能是必須要安裝才能使用=的，如果要用CDN方式使用 Tailwind，需要稍微留意一下下面幾點：
...
<List>
  ...
  <ListItemBad>無法在打包時將沒使用到的樣式忽略</ListItemBad>
</List>

如果想要 Tailwind的完整功能，你應該要 [以 PostCSS 插件的方式來安裝 Tailwind CSS]

如果想要快速的 demo 一下 Tailwind，或是想測試一下這個框架，你可以用 CDN 來取得最新的預設配置：
...
<p>
  請注意一下，CDN 版是很大的 ( 壓縮後 71.5kB, 原始大小 3020.9kB )。**但這不代表用安裝的方式使用 Tailwind，還會是這個大小。**
</p>
